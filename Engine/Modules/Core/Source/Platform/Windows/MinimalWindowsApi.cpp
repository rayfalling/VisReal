...
#include "Platform/Windows/MinimalWinApi.h"
...
static_assert(static_cast<uint32>(C_INDEX_NONE) == TLS_OUT_OF_INDEXES,
	"TLS_OUT_OF_INDEXES is different from INDEX_NONE, change FWindowsPlatformTLS::AllocTlsSlot() implementation.");
...
static_assert(sizeof(Windows::CRITICAL_SECTION) == sizeof(CRITICAL_SECTION),
	"Size of Windows::CRITICAL_SECTION must match definition in Windows.h");
static_assert(__alignof(Windows::CRITICAL_SECTION) == __alignof(CRITICAL_SECTION),
	"Alignment of Windows::CRITICAL_SECTION must match definition in Windows.h");
...
static_assert(__alignof(Windows::LARGE_INTEGER) == __alignof(LARGE_INTEGER),
	"Alignment of Windows::LARGE_INTEGER must match definition in Windows.h");
...
namespace Windows {
	...
	MINIMAL_WINDOWS_API BOOL WINAPI GetOverlappedResult(HANDLE hFile, LPOVERLAPPED lpOverlapped, LPDWORD lpNumberOfBytesTransferred,
	                                                    BOOL bWait) {
		return ::GetOverlappedResult(hFile, lpOverlapped, lpNumberOfBytesTransferred, bWait);
	}

	MINIMAL_WINDOWS_API BOOL WINAPI WriteFile(HANDLE hFile, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite, LPDWORD lpNumberOfBytesWritten,
	                                          LPOVERLAPPED lpOverlapped) {
		return ::WriteFile(hFile, lpBuffer, nNumberOfBytesToWrite, lpNumberOfBytesWritten, lpOverlapped);
	}

	MINIMAL_WINDOWS_API BOOL WINAPI ReadFile(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead,
	                                         LPOVERLAPPED lpOverlapped) {
